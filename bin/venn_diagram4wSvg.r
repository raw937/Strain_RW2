venn_diagram4 <- function(a, b, c, d, name_a = "A", name_b = "B", name_c = "C", name_d = "D", colors =  c("#542BFD","#FCFF4F","#5CFF4A", "#F63C4B"), name_output = "default", euler=FALSE, special=TRUE){
	# Niels Hanson
	...
	png(filename = paste(name_output,'.png', sep=''))
        grid.draw(output)
        dev.off()
	svg(filename = paste(name_output,'.svg', sep=''))
        grid.draw(output)
        dev.off()
		
		# pack all the sets up and return to the user
		out <- NULL;
# 		a <- c(out, name_a, paste("Total=",length(a),sep=""), a)
# 		a_only <- c(out,paste(name_a,"only",sep="_"),paste("Total=",length(a_only),sep=""), a_only)
# 		a_b <- c(out,paste(name_a,name_b,sep="_"), paste("Total=",length(a_b),sep=""),a_b)
# 		a_c <- c(out,paste(name_a,name_c,sep="_"), paste("Total=",length(a_c),sep=""), a_c)
# 		a_d <- c(out,paste(name_a,name_d,sep="_"), paste("Total=",length(a_d),sep=""), a_d)
# 		b <- c(out,name_b,paste("Total=",length(b),sep=""),b)
# 		b_only <- c(out,paste(name_b,"only",sep="_"),paste("Total=",length(b_only),sep=""),b_only)
# 		b_c <- c(out,paste(name_b,name_c,sep="_"), paste("Total=",length(b_c),sep=""),b_c)
# 		b_d <- c(out,paste(name_b,name_d,sep="_"), paste("Total=",length(b_d),sep=""),b_d)
# 		c <- c(out, name_c,paste("Total=",length(c),sep=""), c)
# 		c_only <- c(out,paste(name_c,"only",sep="_"),paste("Total=",length(c_only),sep=""),c_only)
# 		c_d <- c(out,paste(name_c,name_d,sep="_"), paste("Total=",length(c_d),sep=""),c_d)
# 		d <- c(out, name_d,paste("Total=",length(d),sep=""), d)
# 		d_only <- c(out,paste(name_d,"only",sep="_"),paste("Total=",length(d_only),sep=""),d_only)		
# 		a_b_c <- c(out,paste(name_a,name_b,name_c,sep="_"), paste("Total=",length(a_b_c),sep=""),a_b_c)
# 		b_c_d <- c(out,paste(name_b,name_c,name_d,sep="_"), paste("Total=",length(b_c_d),sep=""),b_c_d)
# 		a_c_d <- c(out,paste(name_a,name_c,name_d,sep="_"), paste("Total=",length(a_c_d),sep=""),a_c_d)
# 		a_b_d <- c(out,paste(name_a,name_b,name_d,sep="_"), paste("Total=",length(a_b_d),sep=""),a_b_d)
# 		a_b_c_d <- c(out,paste(name_a,name_b,name_c,name_d,sep="_"), paste("Total=",length(a_b_c_d),sep=""),a_b_c_d)
